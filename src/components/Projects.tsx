...
export default function Projects({ projectArray, id }: { projectArray: Array<project>, id: string }) {
    ...
    return (
        <section className="w-full flex justify-center items-center snap-center h-screen">
            <section className="flex flex-wrap max-w-8/12 gap-8 justify-center items-center place-items-center group max-h-screen py-16 overflow-auto" id={id} style={(window.innerWidth >= 768) ? { gridTemplateColumns: `repeat(${Math.min((projectArray.length == 4) ? 2 : 3, projectArray.length)}, minmax(0, 1fr))` } : {}} ref={reference}>
                        ...
                        <Fragment key={idx}>
                            ...
                            <section className={` transition-all duration-200 fixed left-1/2 top-1/2 -translate-1/2 w-1/2 ${openPopup == idx ? "visible opacity-100" : "invisible opacity-0 -translate-y-full"} flex flex-col gap-2 justify-between bg-gray-600/80 rounded p-2 z-30`}>
                                <div className="flex justify-between w-full">
                                    <h2 className="flex flex-row gap-2 text-xl">{project.title} {icon}  {<a href={project.link}>GIT</a>}</h2>
