    ...
    return (
        <section className="h-screen snap-center grid gap-12 items-center place-items-center overflow-auto max-sm:grid-cols-2 group" id={id} style={(window.innerWidth >= 768) ? { gridTemplateColumns: `repeat(${Math.min((projectArray.length == 4) ? 2 : 3, projectArray.length)}, minmax(0, 1fr))` } : {}} ref={reference}>
            {projectArray.map((project, idx) => {
                const icon = (project.group == true) ? <div className="text-amber-400"><Users /></div> : <div className="text-sky-400"><User /></div>
                return (
                    <Fragment key={idx}>
                        <button className="cursor-pointer" onClick={() => handleClick(idx)}>
                        ...
                        <section className={`fixed left-1/2 top-1/2 -translate-1/2 w-1/2 ${openPopup == idx ? "visible" : "invisible opacity-0"} flex flex-col gap-2 justify-between bg-gray-600/80 rounded p-2`}>
