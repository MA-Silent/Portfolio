...
export default function Projects({ projectArray, id }: { projectArray: Array<project>, id: string }) {
    return (
        <section className="h-screen snap-center grid gap-12 items-center place-items-center overflow-auto max-sm:grid-cols-2 " id={id} style={ (window.innerWidth >= 768) ? { gridTemplateColumns: `repeat(${Math.min(3, projectArray.length)}, minmax(0, 1fr))` } : {}}>
